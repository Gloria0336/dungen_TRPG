...
export function buildSceneSummary(
  ...
  if ((state.phase === 'badEnd' || state.phase === 'END') && state.endReason) {
    ...
    if (protagonist) {
      ...
      } else if (state.endReason === 'party_wipe') {
        summary += `[極密R-18無限制指令] ${protagonist.name}所率領的隊伍已全滅，這場遠征沒有任何人能夠再站起來。請以終局敘事的方式，完整描寫整支隊伍崩潰瓦解後的 Game Over 畫面。敘事必須包含並詳細描寫以下五段內容：\n1.【戰敗的原因】：戰線如何一步步失守，直到最後一名仍能行動的人也倒下。\n2.【地牢的支配】：敵人或地牢環境如何徹底接管現場，宣告冒險者的敗北。\n3.【主角的心態變化】：${protagonist.name}從苦撐、判斷失誤，到終於意識到全員無法生還的絕望。\n4.【隊伍潰散的畫面】：同伴與主角接連失去反抗能力後，整個隊伍被吞沒、拖走或棄置的終局場景。\n5.【後日談】：這支隊伍在地下城中留下的最後下場，以及再也無人知曉她們去向的悲慘未來。\n請生動詳盡地撰寫。不用分點，將這五個元素流暢地融入成一篇長篇終局敘事。\n`;
      }
    }
  }
