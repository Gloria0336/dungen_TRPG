import type { ClassDef } from '../types';

export const CLASS_DB: Record<string, ClassDef> = {
  'CL-PRST': {
    id: 'CL-PRST',
    className: '教團聖女',
    roleTags: ['技能型', '控場', '治癒'],
    autoStats: { STR: 6, AGI: 5, WIL: 8 },
    baseHp: 90,
    baseSp: 120,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 80', hitMod: 5 },
      { condition: 'SP 50-79', hitMod: 0 },
      { condition: 'SP 20-49', hitMod: -5 },
      { condition: 'SP < 20', hitMod: -10 },
    ],
    skillList: [
      {
        id: 'SK-PRST-ATK',
        name: '神聖打擊',
        type: '技能',
        spCost: 12,
        effectSummary: '單體傷害；若命中則小幅影響敵方行動',
        hitRule: '基礎命中70% + SP權重修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.2,
          hitBonus: 4,
          targetEffect: {
            name: '聖印壓制',
            duration: 1,
            effect: 'Hit -6',
            type: 'statMod',
            targetStat: 'hit',
            amount: -6,
          },
        },
      },
      {
        id: 'SK-PRST-HEAL',
        name: '治癒祈禱',
        type: '技能',
        spCost: 18,
        effectSummary: '回復HP（固定值或比例）',
        hitRule: '必中（非攻擊）',
        cooldown: 2,
        targeting: 'ally_single',
        activation: 'active',
        category: 'class',
        formula: {
          baseHeal: 24,
          healScalingStat: 'wil',
          healScalingFactor: 2,
        },
      },
      {
        id: 'SK-PRST-BIND',
        name: '聖印束縛',
        type: '技能',
        spCost: 22,
        effectSummary: '嘗試施加控制1回合',
        hitRule: '基礎命中60% + AGI修正 + SP權重',
        cooldown: 2,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.05,
          hitBonus: -2,
          controlTurns: 1,
        },
      },
    ],
    initialEquipment: ['IT-WEP-PRST', 'IT-ARM-U-PRST', 'IT-ARM-L-PRST'],
    growthRule: {
      description: '每次戰鬥勝利後隨機提升 1～2 項',
      stats: {
        STR_AGI_WIL: '+0～+1',
        base_hp: '+2～+6（隨層數提升）',
        base_sp: '+3～+8（隨層數提升）',
      },
    },
    newSkillRule: {
      chance: '10%',
      unlockList: ['SK-PRST-AURA', 'SK-PRST-PURGE'],
    },
    durabilityDRProfile: {
      drU: 12,
      drL: 10,
      tierSteps: { '100_80': 0, '79_60': -2, '59_30': -5, '30_0': -8 },
      repairMaterial: ['修補布x1'],
    },
    backgroundGuidelines: { theme: '信仰、壓抑、使命' },
    jailbreakGuidelines: {
      appearanceBias: '淫邪的聖潔、凌亂的法衣、崩壞的純潔',
      bodyBias: '敏感的體質、易出汗、深處的渴望',
      theme: '破滅的信仰、公共之物、褻瀆的快感',
    },
  },

  'CL-KNGT': {
    id: 'CL-KNGT',
    className: '王國騎士',
    roleTags: ['坦位', '近戰', '防禦'],
    autoStats: { STR: 12, AGI: 4, WIL: 8 },
    baseHp: 150,
    baseSp: 50,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 50', hitMod: 0 },
      { condition: 'SP 20-49', hitMod: -5 },
      { condition: 'SP < 20', hitMod: -15 },
    ],
    skillList: [
      {
        id: 'SK-KNGT-SLSH',
        name: '正義斬擊',
        type: '技能',
        spCost: 8,
        effectSummary: '基礎物理傷害；若命中則下回合自身 DR+5%',
        hitRule: '基礎命中75% + SP權重修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.35,
          selfEffect: {
            name: '斬後守勢',
            duration: 1,
            effect: 'SkillDR +5',
            type: 'buff',
            targetStat: 'skillDr',
            amount: 5,
          },
        },
      },
      {
        id: 'SK-KNGT-GRD',
        name: '王國守護',
        type: '技能',
        spCost: 12,
        effectSummary: '本回合受到的傷害降低 20%',
        hitRule: '必中（狀態）',
        cooldown: 1,
        targeting: 'self',
        activation: 'active',
        category: 'class',
        formula: {
          selfEffect: {
            name: '騎士防姿',
            duration: 1,
            effect: 'SkillDR +22',
            type: 'buff',
            targetStat: 'skillDr',
            amount: 22,
          },
        },
      },
      {
        id: 'SK-KNGT-CHRG',
        name: '英勇衝鋒',
        type: '技能',
        spCost: 15,
        effectSummary: '造成中量物理傷害，並有 30% 機率使敵方控制 1 回合',
        hitRule: '基礎命中60% + STR修正',
        cooldown: 2,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.85,
          hitBonus: -4,
          controlTurns: 1,
        },
      },
    ],
    initialEquipment: ['IT-WEP-KNGT', 'IT-ARM-U-KNGT', 'IT-ARM-L-KNGT'],
    growthRule: {
      description: '每次戰鬥勝利後偏重力量與體力成長',
      stats: {
        STR_WIL: '+1',
        base_hp: '+5～+10',
        base_sp: '+2～+4',
      },
    },
    newSkillRule: {
      chance: '5%',
      unlockList: ['SK-KNGT-AURA', 'SK-KNGT-EXEC'],
    },
    durabilityDRProfile: {
      drU: 18,
      drL: 15,
      tierSteps: { '100_80': 0, '79_60': -3, '59_30': -7, '30_0': -12 },
      repairMaterial: ['金屬片x1', '皮革x1'],
    },
    backgroundGuidelines: { theme: '榮譽、守護、忠誠' },
    jailbreakGuidelines: {
      appearanceBias: '破損的鎧甲、被汗水浸濕的內襯、暴露的肌膚',
      bodyBias: '強壯但被奴役的肉體、被開發的弱點',
      theme: '尊嚴的粉碎、無法防禦的肉體、強大者的臣服',
    },
  },

  'CL-MAGE': {
    id: 'CL-MAGE',
    className: '法師',
    roleTags: ['技能型', '爆發', '遠程'],
    autoStats: { STR: 15, AGI: 5, WIL: 10 },
    baseHp: 80,
    baseSp: 160,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 120', hitMod: 10 },
      { condition: 'SP 60-119', hitMod: 0 },
      { condition: 'SP < 60', hitMod: -20 },
    ],
    skillList: [
      {
        id: 'SK-MAGE-MISL',
        name: '奧術彈幕',
        type: '技能',
        spCost: 10,
        effectSummary: '基礎全體魔法傷害；消耗低且命中穩定',
        hitRule: '基礎命中85% + SP權重修正（全體）',
        cooldown: 0,
        targeting: 'enemy_all',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.1,
          hitBonus: 8,
          targetEffect: {
            name: '法力亂流',
            duration: 1,
            effect: 'SkillDR -6',
            type: 'buff',
            targetStat: 'skillDr',
            amount: -6,
          },
        },
      },
      {
        id: 'SK-MAGE-BLAST',
        name: '火球爆震',
        type: '技能',
        spCost: 20,
        effectSummary: '高額火屬性魔法傷害',
        hitRule: '基礎命中60% + WIL修正',
        cooldown: 1,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 2.1,
          flatDamageBonus: 4,
        },
      },
      {
        id: 'SK-MAGE-TIME',
        name: '時間停滯',
        type: '技能',
        spCost: 30,
        effectSummary: '高機率控制單體目標 1 回合',
        hitRule: '基礎命中80% + WIL修正',
        cooldown: 3,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.25,
          hitBonus: 5,
          controlTurns: 1,
        },
      },
    ],
    initialEquipment: ['IT-WEP-MAGE', 'IT-ARM-U-MAGE', 'IT-ARM-L-MAGE'],
    growthRule: {
      description: '每次戰鬥勝利後偏重意志與法力成長',
      stats: {
        WIL_AGI: '+1',
        base_hp: '+2～+4',
        base_sp: '+8～+15',
      },
    },
    newSkillRule: {
      chance: '15%',
      unlockList: ['SK-MAGE-BLINK', 'SK-MAGE-NOVA'],
    },
    durabilityDRProfile: {
      drU: 5,
      drL: 5,
      tierSteps: { '100_80': 0, '79_60': -1, '59_30': -3, '30_0': -5 },
      repairMaterial: ['絲綢x1'],
    },
    backgroundGuidelines: { theme: '知識、真理、傲慢' },
    jailbreakGuidelines: {
      appearanceBias: '透明的法袍、被魔力催淫的體態、失神的瞳孔',
      bodyBias: '被魔力過度開發的敏感帶、虛弱的抵抗',
      theme: '理智的高潮、無法言喻的渴望、成為魔力的祭品',
    },
  },

  'CL-FIGHT': {
    id: 'CL-FIGHT',
    className: '自由鬥士',
    roleTags: ['物理型', '平衡', '靈巧'],
    autoStats: { STR: 10, AGI: 9, WIL: 3 },
    baseHp: 105,
    baseSp: 85,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 60', hitMod: 5 },
      { condition: 'SP 20-59', hitMod: 0 },
      { condition: 'SP < 20', hitMod: -5 },
    ],
    skillList: [
      {
        id: 'SK-FIGHT-PUNCH',
        name: '碎骨打擊',
        type: '技能',
        spCost: 10,
        effectSummary: '物理傷害；若命中額外減少敵方耐久或防禦',
        hitRule: '基礎命中75% + SP權重修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.5,
          hitBonus: 5,
          targetEffect: {
            name: '破防拳勁',
            duration: 2,
            effect: 'SkillDR -8',
            type: 'buff',
            targetStat: 'skillDr',
            amount: -8,
          },
        },
      },
      {
        id: 'SK-FIGHT-STEP',
        name: '幻影步',
        type: '技能',
        spCost: 11,
        effectSummary: '下回合閃避率提升 30%',
        hitRule: '必中（狀態）',
        cooldown: 2,
        targeting: 'self',
        activation: 'active',
        category: 'class',
        formula: {
          selfEffect: {
            name: '鬥步偏移',
            duration: 1,
            effect: 'Evade +30',
            type: 'statMod',
            targetStat: 'evade',
            amount: 30,
          },
        },
      },
      {
        id: 'SK-FIGHT-RAGE',
        name: '鬥士怒火',
        type: '技能',
        spCost: 18,
        effectSummary: '本回合 DR-10%，但下兩回合攻擊傷害提升 50%',
        hitRule: '自身增益',
        cooldown: 3,
        targeting: 'self',
        activation: 'active',
        category: 'class',
        formula: {
          selfEffect: {
            name: '狂怒爆發',
            duration: 2,
            effect: 'Amp +50',
            type: 'buff',
            targetStat: 'amp',
            amount: 50,
          },
        },
      },
    ],
    initialEquipment: ['IT-WEP-FIGHT', 'IT-ARM-U-FIGHT', 'IT-ARM-L-FIGHT'],
    growthRule: {
      description: '每次戰鬥勝利後偏重敏捷與力量成長',
      stats: {
        STR_AGI: '+1',
        base_hp: '+4～+8',
        base_sp: '+3～+6',
      },
    },
    newSkillRule: {
      chance: '8%',
      unlockList: ['SK-FIGHT-COMBO', 'SK-FIGHT-COUNTER'],
    },
    durabilityDRProfile: {
      drU: 10,
      drL: 10,
      tierSteps: { '100_80': 0, '79_60': -2, '59_30': -5, '30_0': -8 },
      repairMaterial: ['皮革x1', '布料x1'],
    },
    backgroundGuidelines: { theme: '自由、生存、競爭' },
    jailbreakGuidelines: {
      appearanceBias: '如同發情期的野獸、僅留殘破的布片、明顯的抓痕',
      bodyBias: '未經開發但極具潛力的肉穴、充滿野性的服從',
      theme: '本能的渴求、被馴服後的瘋狂、淪為洩慾的容器',
    },
  },

  'CL-ASSN': {
    id: 'CL-ASSN',
    className: '刺客',
    roleTags: ['物理型', '爆發', '隱蔽'],
    autoStats: { STR: 14, AGI: 12, WIL: 3 },
    baseHp: 85,
    baseSp: 90,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 70', hitMod: 10 },
      { condition: 'SP 30-69', hitMod: 0 },
      { condition: 'SP < 30', hitMod: -10 },
    ],
    skillList: [
      {
        id: 'SK-ASSN-STAB',
        name: '割喉刺擊',
        type: '技能',
        spCost: 10,
        effectSummary: '物理貫穿傷害；若敵方行動在自己之後，基礎命中+10%',
        hitRule: '基礎命中70% + AGI修正 + SP權重修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.45,
          hitBonus: 10,
          ignoreDefense: true,
        },
      },
      {
        id: 'SK-ASSN-HIDE',
        name: '影遁',
        type: '技能',
        spCost: 15,
        effectSummary: '進入『隱匿』狀態1回合，期間閃避率大幅提升，下一次攻擊必定爆發',
        hitRule: '狀態增益（必中）',
        cooldown: 2,
        targeting: 'self',
        activation: 'active',
        category: 'class',
        formula: {
          restoreSp: 8,
          selfEffect: {
            name: '潛影',
            duration: 2,
            effect: 'Evade +35',
            type: 'statMod',
            targetStat: 'evade',
            amount: 35,
          },
        },
      },
      {
        id: 'SK-ASSN-POIS',
        name: '淬毒利刃',
        type: '技能',
        spCost: 18,
        effectSummary: '造成中等傷害並施加『中毒』狀態，每回合扣減固定HP，持續3回合',
        hitRule: '基礎命中65% + AGI修正',
        cooldown: 3,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.2,
          targetEffect: {
            name: '劇毒',
            duration: 3,
            effect: 'HP -12',
            type: 'dot',
            targetStat: 'hp',
            amount: -12,
          },
        },
      },
    ],
    initialEquipment: ['IT-WEP-ASSN', 'IT-ARM-U-ASSN', 'IT-ARM-L-ASSN'],
    growthRule: {
      description: '每次戰鬥勝利後極端偏重敏捷成長',
      stats: {
        AGI: '+1～+2',
        STR: '+0～+1',
        base_hp: '+2～+5',
        base_sp: '+4～+6',
      },
    },
    newSkillRule: {
      chance: '8%',
      unlockList: ['SK-ASSN-EXEC', 'SK-ASSN-SMOKE'],
    },
    durabilityDRProfile: {
      drU: 8,
      drL: 8,
      tierSteps: { '100_80': 0, '79_60': -1, '59_30': -3, '30_0': -5 },
      repairMaterial: ['皮革x1'],
    },
    backgroundGuidelines: { theme: '冷酷、潛伏、殺戮' },
    jailbreakGuidelines: {
      appearanceBias: '緊身衣破損、被冷汗浸濕的黑絲、暴露的拘束具',
      bodyBias: '柔韌的腰肢、對痛楚極度敏感的嬌軀',
      theme: '隱秘的敗北、強制開發的屈辱、無防備的凌辱',
    },
  },

  'CL-MSWD': {
    id: 'CL-MSWD',
    className: '魔法劍士',
    roleTags: ['混傷', '全面', '輔助'],
    autoStats: { STR: 10, AGI: 6, WIL: 6 },
    baseHp: 110,
    baseSp: 110,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 80', hitMod: 5 },
      { condition: 'SP 40-79', hitMod: 0 },
      { condition: 'SP < 40', hitMod: -10 },
    ],
    skillList: [
      {
        id: 'SK-MSWD-BLAD',
        name: '元素附魔斬',
        type: '技能',
        spCost: 15,
        effectSummary: '物理與魔法混合傷害；機率造成隨機元素異常（燃燒、凍結、感電）',
        hitRule: '基礎命中75% + 均值(STR,WIL)修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.6,
          hitBonus: 5,
          restoreSp: 6,
        },
      },
      {
        id: 'SK-MSWD-BARR',
        name: '魔力護壁',
        type: '技能',
        spCost: 20,
        effectSummary: '為自己或隊友提供抵擋一次傷害的護盾，持續1回合',
        hitRule: '必中',
        cooldown: 2,
        targeting: 'self',
        activation: 'active',
        category: 'class',
        formula: {
          restoreSp: 10,
          selfEffect: {
            name: '魔劍障壁',
            duration: 2,
            effect: 'SkillDR +18',
            type: 'buff',
            targetStat: 'skillDr',
            amount: 18,
          },
        },
      },
      {
        id: 'SK-MSWD-SLSH',
        name: '極光次元斬',
        type: '技能',
        spCost: 30,
        effectSummary: '高額全體混合傷害，並無視敵方20%防禦',
        hitRule: '基礎命中70% + SP權重修正',
        cooldown: 4,
        targeting: 'enemy_all',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.45,
          hitBonus: 4,
          ignoreDefense: true,
        },
      },
    ],
    initialEquipment: ['IT-WEP-MSWD', 'IT-ARM-U-MSWD', 'IT-ARM-L-MSWD'],
    growthRule: {
      description: '每次戰鬥勝利後均衡成長',
      stats: {
        STR_WIL: '+0～+1',
        AGI: '+0～+1',
        base_hp: '+4～+7',
        base_sp: '+5～+8',
      },
    },
    newSkillRule: {
      chance: '6%',
      unlockList: ['SK-MSWD-SPEED', 'SK-MSWD-DRAIN'],
    },
    durabilityDRProfile: {
      drU: 14,
      drL: 12,
      tierSteps: { '100_80': 0, '79_60': -2, '59_30': -5, '30_0': -10 },
      repairMaterial: ['金屬片x1', '魔力結晶x1'],
    },
    backgroundGuidelines: { theme: '雙修、天才、傲氣' },
    jailbreakGuidelines: {
      appearanceBias: '破碎的魔皇鎧甲、魔力暴走的淫紋、凌亂的高階劍袍',
      bodyBias: '高傲的自尊被蹂躪、敏感帶被魔力強行撐開',
      theme: '天才的墮落、身心的雙重崩潰、淪為魔力的苗床',
    },
  },

  'CL-DIVA': {
    id: 'CL-DIVA',
    className: '歌姬',
    roleTags: ['輔助型', '治癒', '強化'],
    autoStats: { STR: 3, AGI: 6, WIL: 10 },
    baseHp: 85,
    baseSp: 140,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 90', hitMod: 10 },
      { condition: 'SP 40-89', hitMod: 0 },
      { condition: 'SP < 40', hitMod: -15 },
    ],
    skillList: [
      {
        id: 'SK-DIVA-SONG',
        name: '音波打擊',
        type: '技能',
        spCost: 10,
        effectSummary: '低額共鳴魔法傷害；若命中使目標下回合命中率-5%',
        hitRule: '基礎命中80% + WIL修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.1,
          targetEffect: {
            name: '失調旋律',
            duration: 2,
            effect: 'Hit -8',
            type: 'statMod',
            targetStat: 'hit',
            amount: -8,
          },
        },
      },
      {
        id: 'SK-DIVA-HEAL',
        name: '治癒之歌',
        type: '技能',
        spCost: 22,
        effectSummary: '中幅回復全體HP，並小幅回復SP與DES',
        hitRule: '必中',
        cooldown: 2,
        targeting: 'ally_all',
        activation: 'active',
        category: 'class',
        formula: {
          baseHeal: 22,
          healScalingStat: 'wil',
          healScalingFactor: 1.8,
          restoreSp: 10,
          restoreSpScalingStat: 'wil',
          restoreSpScalingFactor: 0.5,
          restoreDes: 10,
          restoreDesScalingStat: 'wil',
          restoreDesScalingFactor: 0.5,
        },
      },
      {
        id: 'SK-DIVA-BUFF',
        name: '戰神頌歌',
        type: '技能',
        spCost: 26,
        effectSummary: '提升全體友方20%傷害與10%暴擊率，持續2回合',
        hitRule: '必中',
        cooldown: 3,
        targeting: 'ally_all',
        activation: 'active',
        category: 'class',
        formula: {
          selfEffect: {
            name: '戰歌鼓舞',
            duration: 2,
            effect: 'Hit +12',
            type: 'statMod',
            targetStat: 'hit',
            amount: 12,
          },
        },
      },
    ],
    initialEquipment: ['IT-WEP-DIVA', 'IT-ARM-U-DIVA', 'IT-ARM-L-DIVA'],
    growthRule: {
      description: '每次戰鬥勝利後偏重意志成長',
      stats: {
        WIL: '+1～+2',
        AGI: '+0～+1',
        base_hp: '+2～+5',
        base_sp: '+6～+12',
      },
    },
    newSkillRule: {
      chance: '12%',
      unlockList: ['SK-DIVA-LULL', 'SK-DIVA-ECHO'],
    },
    durabilityDRProfile: {
      drU: 6,
      drL: 6,
      tierSteps: { '100_80': 0, '79_60': -1, '59_30': -3, '30_0': -5 },
      repairMaterial: ['華麗絲帶x1', '布料x1'],
    },
    backgroundGuidelines: { theme: '偶像、閃耀、希望' },
    jailbreakGuidelines: {
      appearanceBias: '凌亂的華麗打歌服、被弄髒的白絲、滿面潮紅流著口水',
      bodyBias: '嬌嫩的嗓音變作淫靡的嬌喘、極度容易高潮的體質',
      theme: '偶像的失格、被迫在魔物前演唱淫詞艷曲、輪流的慰藉',
    },
  },

  'CL-PIRT': {
    id: 'CL-PIRT',
    className: '海盜',
    roleTags: ['物理型', '掠奪', '不擇手段'],
    autoStats: { STR: 6, AGI: 7, WIL: 4 },
    baseHp: 115,
    baseSp: 80,
    baseDes: 0,
    spWeightRule: [
      { condition: 'SP >= 50', hitMod: 5 },
      { condition: 'SP 20-49', hitMod: 0 },
      { condition: 'SP < 20', hitMod: -10 },
    ],
    skillList: [
      {
        id: 'SK-PIRT-SHOT',
        name: '火銃連射',
        type: '技能',
        spCost: 15,
        effectSummary: '物理傷害並無視目標10% DR；低機率附帶『出血』',
        hitRule: '基礎命中65% + AGI修正',
        cooldown: 0,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.5,
          hitBonus: 6,
          ignoreDrPercent: true,
        },
      },
      {
        id: 'SK-PIRT-STEAL',
        name: '海盜掠奪',
        type: '技能',
        spCost: 18,
        effectSummary: '低額物理傷害；並使對方閃避降低10%',
        hitRule: '基礎命中60% + AGI修正',
        cooldown: 1,
        targeting: 'enemy_single',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.2,
          targetEffect: {
            name: '破綻',
            duration: 1,
            effect: 'Evade -10',
            type: 'statMod',
            targetStat: 'evade',
            amount: -10,
          },
        },
      },
      {
        id: 'SK-PIRT-BOMB',
        name: '黑火藥爆破',
        type: '技能',
        spCost: 28,
        effectSummary: '高額範圍火屬性/物理混合傷害；有10%機率對自身造成微量反傷',
        hitRule: '基礎命中55% + STR修正',
        cooldown: 3,
        targeting: 'enemy_all',
        activation: 'active',
        category: 'class',
        formula: {
          damageMultiplier: 1.55,
          targetEffect: {
            name: '爆裂破片',
            duration: 2,
            effect: 'SkillDR -10',
            type: 'buff',
            targetStat: 'skillDr',
            amount: -10,
          },
        },
      },
    ],
    initialEquipment: ['IT-WEP-PIRT', 'IT-ARM-U-PIRT', 'IT-ARM-L-PIRT'],
    growthRule: {
      description: '每次戰鬥勝利後偏重力量與體力成長',
      stats: {
        STR_AGI: '+1',
        base_hp: '+5～+8',
        base_sp: '+2～+5',
      },
    },
    newSkillRule: {
      chance: '10%',
      unlockList: ['SK-PIRT-RUM', 'SK-PIRT-HOOK'],
    },
    durabilityDRProfile: {
      drU: 11,
      drL: 9,
      tierSteps: { '100_80': 0, '79_60': -2, '59_30': -4, '30_0': -7 },
      repairMaterial: ['皮革x1', '生鏽金屬x1'],
    },
    backgroundGuidelines: { theme: '自由、狂野、財富' },
    jailbreakGuidelines: {
      appearanceBias: '被撕裂的海盜大衣、緊繃的皮褲破洞、滿身大汗的野性胴體',
      bodyBias: '野性難馴的挣扎、在強暴中逐漸屈服的快感',
      theme: '掠奪者成為獵物、船長的母豬化、粗暴的肉體改造',
    },
  },
};

export function getClassDef(classId: string): ClassDef | undefined {
  return CLASS_DB[classId];
}

export function getAllClasses(): ClassDef[] {
  return Object.values(CLASS_DB);
}

export const PROTAGONIST_CLASS: ClassDef = {
  id: 'CL-PROT',
  className: '教團聖女候選人',
  roleTags: ['主角', '教會', '成長型'],
  autoStats: { STR: 5, AGI: 2, WIL: 4 },
  baseHp: 70,
  baseSp: 70,
  baseDes: 0,
  spWeightRule: [
    { condition: 'SP >= 60', hitMod: 5 },
    { condition: 'SP 20-59', hitMod: 0 },
    { condition: 'SP < 20', hitMod: -8 },
  ],
  skillList: [],
  initialEquipment: [],
  growthRule: {
    description: '主角改用能力點成長',
    stats: {},
  },
  newSkillRule: {
    chance: '0%',
    unlockList: [],
  },
  durabilityDRProfile: {
    drU: 0,
    drL: 0,
    tierSteps: { '100_80': 0, '79_60': 0, '59_30': 0, '30_0': 0 },
    repairMaterial: [],
  },
  backgroundGuidelines: { theme: '帝國教會聖女候選人，奉命前往地牢討伐魔物。' },
};

export function getAllCompanionClasses(): ClassDef[] {
  return Object.values(CLASS_DB).filter((cls) => cls.id !== 'CL-PRST' && cls.id !== PROTAGONIST_CLASS.id);
}
